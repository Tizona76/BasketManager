...
blason_img = pygame.image.load("images/blason.png").convert_alpha()
blason_img = pygame.transform.scale(blason_img, (100, 100))  # Adapter la taille si besoin
...
def simuler_match():
    ...
    resultats_matchs_complets.append({
        ...
        "mvp_match": {
         ...
         # Gestion de l'ouverture du mercato
    global mercato_ouvert, mercato_matchs_restants
...
def dessiner_mon_equipe():
    ...
    # Coordonnées personnalisées pour le blason (modifie x_blason et y_blason selon ton besoin)
    x_blason = LARGEUR // 2      
    y_blason = 50               

    largeur_blason = 120
    hauteur_blason = 120
    blason_redim = pygame.transform.scale(blason_img, (largeur_blason, hauteur_blason))
    blason_rect = blason_redim.get_rect(center=(x_blason, y_blason))
    FENETRE.blit(blason_redim, blason_rect)
...
def dessiner_finances():
    ...
    total_previsionnel_billetterie = total_billetterie
    total_previsionnel_boutique = total_boutique

    coef_pop = max(0.3, min(popularite / 100, 1.0))

    recettes_billetterie_cumulees = int(total_previsionnel_billetterie * coef_pop)
    recettes_boutique_cumulees = int(total_previsionnel_boutique * coef_pop)
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if etat == "billetterie":
                    ...
                    if champ_texte.collidepoint(pos):
                        saisie_active = True
                        blason_rect = blason_img.get_rect(center=(LARGEUR // 2, 50))
                    FENETRE.blit(blason_img, blason_rect)
