...
total_billetterie = 0
total_boutique = 0
total_billetterie_cumulee = 0
total_boutique_cumulee = 0
...
def dessiner_finances():
    ...
    total_charges_salaire = sum(
        int(joueurs[i]['salaire'].replace(' €', '').replace(' ', '').replace('.', '')) for i in equipe
    )
    total_entretien = 75_500

    total_recettes_calcule = recettes_sponsors + total_billetterie + total_boutique
    ...
    solde_final = total_recettes_calcule - total_depenses_calcule

    couleur = (50, 160, 50) if solde_final >= 0 else (200, 50, 50)
    texte_police = pygame.font.SysFont("comicsansms", 24, bold=True)
    clignoter = True
    if solde_final < 0:
        temps_actuel = pygame.time.get_ticks()
        clignoter = (temps_actuel // 400) % 2 == 0

    if clignoter:
        texte_surface = texte_police.render(f"Solde final : {solde_final:,} €".replace(",", "."), True, couleur)
    else:
        texte_surface = texte_police.render("Solde final :", True, couleur)

    rect_texte = texte_surface.get_rect(topleft=(380, 580))
    pygame.draw.rect(FENETRE, couleur, rect_texte.inflate(20, 10), 2)
    ...
    for i, (titre, valeur, couleur) in enumerate(categories):
        ...
        pygame.draw.rect(FENETRE, couleur, rect)
        pygame.draw.rect(FENETRE, NOIR, rect, 2)
        texte = pygame.font.SysFont("comicsansms", 22, bold=True).render(
            f"{titre} : {valeur:,} €".replace(",", "."), True, NOIR
        )
    ...
    # Détail sponsors
    pygame.draw.rect(FENETRE, (180, 255, 180), (x_charges, y_charges, largeur_charges, hauteur_charges))
    pygame.draw.rect(FENETRE, NOIR, (x_charges, y_charges, largeur_charges, hauteur_charges), 2)
    texte_sponsor = petite_font.render(
        f"Détail Sponsors : {montant_sponsor_actuel:,} €".replace(",", "."), True, NOIR
    )
    FENETRE.blit(texte_sponsor, (x_charges + 10, y_charges + 10))

    # Détail billetterie pondérée
    y_charges += 60
    try:
        prix_a = int(saisies_billet[0])
        prix_b = int(saisies_billet[1])
    ...
    total_previsionnel_billetterie = (prix_a + prix_b + prix_c) * places_par_cat
    taux_pop = max(0.3, min(popularite / 100, 1))
    billet_pondere = int(total_previsionnel_billetterie * taux_pop)

    pygame.draw.rect(FENETRE, (180, 255, 180), (x_charges, y_charges, largeur_charges, hauteur_charges))
    pygame.draw.rect(FENETRE, NOIR, (x_charges, y_charges, largeur_charges, hauteur_charges), 2)
    texte_billetterie = petite_font.render(
        f"Détail Billetterie : {billet_pondere:,} €".replace(",", "."), True, NOIR
    )
    FENETRE.blit(texte_billetterie, (x_charges + 10, y_charges + 10))

    # Détail boutique pondérée
    y_charges += 60
    try:
        prix_casquette = int(saisies_prix_boutique[0])
        prix_echarpe = int(saisies_prix_boutique[1])
    ...
    boutique_pondere = int(total_previsionnel_boutique * taux_pop)

    pygame.draw.rect(FENETRE, (180, 255, 180), (x_charges, y_charges, largeur_charges, hauteur_charges))
    pygame.draw.rect(FENETRE, NOIR, (x_charges, y_charges, largeur_charges, hauteur_charges), 2)
    texte_boutique = petite_font.render(
        f"Détail Boutique : {boutique_pondere:,} €".replace(",", "."), True, NOIR
    )
    FENETRE.blit(texte_boutique, (x_charges + 10, y_charges + 10))

    # Recette dernier match
    y_charges += 115
    recette_match = total_billetterie + total_boutique if len(historique_matchs) % 2 == 0 else 0
    rect_match = pygame.Rect(x_charges, y_charges, largeur_match, hauteur_match)
    ...
    FENETRE.blit(titre_match, (x_charges + 20, y_charges + 0))
    FENETRE.blit(montant, (x_charges + 100, y_charges + 20))

    # Détail charges
    x_charges = 590
    y_charges = 330
    for label, montant in [
        ("Salaires", total_charges_salaire),
        ("Entretien stade", COUT_ENTRETIEN_ANNUEL_NIVEAU1),
    ]:
        rect = pygame.Rect(x_charges, y_charges, largeur_charges, hauteur_charges)
        pygame.draw.rect(FENETRE, (255, 180, 180), rect)
        pygame.draw.rect(FENETRE, NOIR, rect, 2)
        montant_affiche = f"{montant:,}".replace(",", ".") + " €"
        texte = petite_font.render(f"{label} : {montant_affiche}", True, NOIR)
        FENETRE.blit(texte, (x_charges + 10, y_charges + 10))
        y_charges += 60

    # Budget total
    rect_total = pygame.Rect(525, 120, 365, 60)
    pygame.draw.rect(FENETRE, (255, 230, 150), rect_total)
    pygame.draw.rect(FENETRE, NOIR, rect_total, 2)
    grande_police = pygame.font.SysFont("comicsansms", 24, bold=True)
    texte_total = grande_police.render(
        f"Budget actuel : {solde_final:,} €".replace(",", "."), True, NOIR
    )
