...
var _save_hover_tooltip: PanelContainer = null
var _save_hover_tooltip_label: Label = null
...
func _bm_scale_mobile_button(btn: Button) -> void:
	if btn == null or not _bm_is_mobile_layout():
		return
	if btn.has_meta("bm_mobile_plus10"):
		return
	btn.scale *= 1.1
	btn.set_meta("bm_mobile_plus10", true)
...
func _ensure_save_hover_tooltip() -> void:
	if _save_hover_tooltip != null and is_instance_valid(_save_hover_tooltip):
		return
	_save_hover_tooltip = PanelContainer.new()
	_save_hover_tooltip.name = "SaveHoverTooltip"
	_save_hover_tooltip.visible = false
	_save_hover_tooltip.mouse_filter = Control.MOUSE_FILTER_IGNORE
	_save_hover_tooltip.z_index = 50
	add_child(_save_hover_tooltip)
	...
	_save_hover_tooltip.add_theme_stylebox_override("panel", sb)

	_save_hover_tooltip_label = Label.new()
	_save_hover_tooltip_label.name = "SaveHoverTooltipLabel"
	_save_hover_tooltip_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	_save_hover_tooltip_label.custom_minimum_size = Vector2(280, 0)
	_save_hover_tooltip_label.add_theme_font_size_override("font_size", 20)
	_save_hover_tooltip_label.add_theme_color_override("font_color", Color(1, 1, 1, 1))
	_save_hover_tooltip.add_child(_save_hover_tooltip_label)

func _show_save_hover_tooltip() -> void:
	_ensure_save_hover_tooltip()
	if _save_hover_tooltip == null or _save_hover_tooltip_label == null or BtnSave == null:
		return
	_save_hover_tooltip_label.text = tr("menu.save.tooltip")
	_save_hover_tooltip.visible = true
	await get_tree().process_frame
	var pos := BtnSave.global_position + Vector2(BtnSave.size.x + 12.0, (BtnSave.size.y * 0.5) - (_save_hover_tooltip.size.y * 0.5) + 20.0)
	_save_hover_tooltip.global_position = pos

func _show_ranking_hover_tooltip() -> void:
	_ensure_save_hover_tooltip()
	if _save_hover_tooltip == null or _save_hover_tooltip_label == null or BtnTabWinrates == null:
		return
	var txt := str(BtnTabWinrates.tooltip_text).strip_edges()
	if txt == "":
		txt = tr("menu.tab.ranking")
	_save_hover_tooltip_label.text = txt
	_save_hover_tooltip.visible = true
	await get_tree().process_frame
	var pos := BtnTabWinrates.global_position + Vector2(BtnTabWinrates.size.x + 12.0, (BtnTabWinrates.size.y * 0.5) - (_save_hover_tooltip.size.y * 0.5) + 20.0)
	_save_hover_tooltip.global_position = pos

func _hide_save_hover_tooltip() -> void:
	if _save_hover_tooltip != null:
		_save_hover_tooltip.visible = false

func _bind_save_hover_tooltip() -> void:
	if BtnSave == null:
		return
	if not BtnSave.mouse_entered.is_connected(_show_save_hover_tooltip):
		BtnSave.mouse_entered.connect(_show_save_hover_tooltip)
	if not BtnSave.mouse_exited.is_connected(_hide_save_hover_tooltip):
		BtnSave.mouse_exited.connect(_hide_save_hover_tooltip)

func _bind_ranking_hover_tooltip() -> void:
	if BtnTabWinrates == null:
		return
	if not BtnTabWinrates.mouse_entered.is_connected(_show_ranking_hover_tooltip):
		BtnTabWinrates.mouse_entered.connect(_show_ranking_hover_tooltip)
	if not BtnTabWinrates.mouse_exited.is_connected(_hide_save_hover_tooltip):
		BtnTabWinrates.mouse_exited.connect(_hide_save_hover_tooltip)
...
func _bm_club_popup_fallback(key: String) -> String:
	...
	var data := {
		"popup_intro_title": {
			"fr": "Build Your Roster",
			"en": "Build Your Roster",
			"es": "Build Your Roster",
			"it": "Build Your Roster",
			"pt": "Build Your Roster"
		},
		...
		"popup_intro_build_body": {
			"fr": "Start by selecting the players for your club : compare their attributes and salaries to build your roster.\nFrom game 5, you'll choose your lineup with these players.",
			"en": "Start by selecting the players for your club : compare their attributes and salaries to build your roster.\nFrom game 5, you'll choose your lineup with these players.",
			"es": "Start by selecting the players for your club : compare their attributes and salaries to build your roster.\nFrom game 5, you'll choose your lineup with these players.",
			"it": "Start by selecting the players for your club : compare their attributes and salaries to build your roster.\nFrom game 5, you'll choose your lineup with these players.",
			"pt": "Start by selecting the players for your club : compare their attributes and salaries to build your roster.\nFrom game 5, you'll choose your lineup with these players."
		},
...
func _bm_menu_play_season_text() -> String:
	...
	if typeof(save_any) == TYPE_DICTIONARY:
		...
		season_number = maxi(1, int(d.get("season_number", 1)))
	var txt := tr("menu.play_season")
	if txt == "menu.play_season" or txt.strip_edges() == "":
		txt = "Play Saison {season}"
	return txt.replace("{season}", str(season_number))


func _bm_refresh_popup_cta_button() -> void:
	if btn_close_bienvenue_club != null:
		btn_close_bienvenue_club.text = _bm_popup_cta_text()
		_bm_apply_mobile_popup_cta_button_size()
...
func _bm_apply_mobile_popup_cta_button_size() -> void:
	...
	if OS.has_feature("web") and not is_mobile_popup_cta:
		...
		is_mobile_popup_cta = bool(js_mobile_popup_cta)
	if not is_mobile_popup_cta:
		return
	if btn_close_bienvenue_club == null:
		return
...
func _on_close_bienvenue_club_pressed() -> void:
	...
	if typeof(save) == TYPE_DICTIONARY:
		save["popup_bienvenue_club_deja_vu"] = true
...
func _ready() -> void:
	...
	call_deferred("_bm_mobile_start_by_building_text_plus2")
	call_deferred("_bm_menu_apply_mobile_hud_plus15")
	call_deferred("_bm_menu_apply_mobile_hud_width_minus20")
	call_deferred("_bm_menu_apply_mobile_landscape_right_side_positions")
	call_deferred("_bm_menu_apply_mobile_save_plus15_text_plus2")
	...
	if BtnTabWinrates != null:
		...
		_bm_apply_play_game_button_style(BtnTabWinrates, Vector2(180, 48))
	...
	if BtnShopTokensDebug != null:
		BtnShopTokensDebug.visible = SHOP_DEBUG_VISIBLE
		BtnShopTokensDebug.disabled = not SHOP_DEBUG_VISIBLE
		if SHOP_DEBUG_VISIBLE and not BtnShopTokensDebug.pressed.is_connected(_on_btn_shop_tokens_debug):
			BtnShopTokensDebug.pressed.connect(_on_btn_shop_tokens_debug)

	call_deferred("_bm_apply_mobile_management_buttons_plus10")
	call_deferred("_bm_menu_apply_mobile_management_buttons_text_plus2")
	call_deferred("_bm_apply_mobile_popup_cta_button_size")
...
func _bm_get_saved_crest_id_menu(save: Dictionary) -> String:
...
func _bm_tween_management_crest_scale(icon: TextureRect, target_scale: Vector2) -> void:
	if icon == null or not is_instance_valid(icon):
		return
	var tw := create_tween()
	tw.set_trans(Tween.TRANS_SINE)
	tw.set_ease(Tween.EASE_OUT)
	tw.tween_property(icon, "scale", target_scale, 0.12)


func _bm_update_club_crest_header_menu(save: Dictionary) -> void:
	if LblClubName == null:
		return

	var icon := get_node_or_null("UI/ClubCrestHeaderIcon") as TextureRect
	var cid: String = _bm_get_saved_crest_id_menu(save)
	print("[MENU CREST DEBUG] cid=", cid, " root=", save.get("club_crest_id", ""), " selected=", save.get("selected_crest_id", ""), " roster=", (save.get("roster", {}) as Dictionary).get("selected_crest_id", ""), " LblClubName pos=", LblClubName.position, " size=", LblClubName.size, " visible=", LblClubName.visible)
	...
	var team_name_for_crest: String = str(save.get("team_name", save.get("club_name", ""))).strip_edges()
	var crest_path: String = PL.get_display_crest_path(save, team_name_for_crest)
	print("[MENU CREST DEBUG] path=", crest_path, " exists=", ResourceLoader.exists(crest_path))
	...
	if icon == null:
		...
		icon.mouse_entered.connect(func() -> void:
			_bm_tween_management_crest_scale(icon, Vector2(1.80, 1.80))
		)
		icon.mouse_exited.connect(func() -> void:
			_bm_tween_management_crest_scale(icon, Vector2.ONE)
		)
		icon.gui_input.connect(func(event):
			if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
				_open_club_identity_popup()
		)

	icon.texture = load(crest_path)
	icon.visible = LblClubName.visible
	icon.size = Vector2(89, 89)
	icon.pivot_offset = icon.size * 0.5

	var name_pos: Vector2 = LblClubName.position
	var name_size: Vector2 = LblClubName.size
	var team_name: String = str(save.get("team_name", "BM Club")).strip_edges()
	if team_name == "":
		team_name = "BM Club"

	var approx_text_w: float = maxf(80.0, float(team_name.length()) * 29.0)
	var text_center_x: float = name_pos.x + name_size.x * 0.5
	var text_left_x: float = text_center_x - approx_text_w * 0.5
	var x: float = maxf(12.0, text_left_x - 66.0)
	var y: float = name_pos.y + (name_size.y - icon.size.y) * 0.5
...
func _on_btn_shop_tokens_debug() -> void:
	print("[SHOP][DEBUG] BtnShopTokensDebug pressed")
	var tree := get_tree()
	if tree == null:
		print("[SHOP][DEBUG][ERR] get_tree() is null")
		return
	var path := "res://scenes/ShopTokens.tscn"
	print("[SHOP][DEBUG] change_scene_to_file path=", path, " exists=", ResourceLoader.exists(path))
	if ResourceLoader.exists(path):
		var err := tree.change_scene_to_file(path)
		print("[SHOP][DEBUG] change_scene_to_file err=", err)
	else:
...
func _bm_can_show_coachs_button() -> bool:
	return PL.is_coachs_unlocked(PL.load_savegame())
...
func _tr_safe(key: String) -> String:
	...
	match key:
		...
		"menu.save_choice.title":
			match _bm_club_lang_code():
				"fr": return "Sauvegarder votre progression"
				"es": return "Guardar tu progreso"
				"it": return "Salva i tuoi progressi"
				"pt": return "Guardar o teu progresso"
		...
		"menu.save_choice.cloud_tip":
			match _bm_club_lang_code():
				...
				"es": return "Guardar online para proteger mejor tu progreso."
				"it": return "Salva online per proteggere meglio i tuoi progressi."
				"pt": return "Guardar online para proteger melhor o teu progresso."
				_: return "Save online and keep your progress safer."
		_:
			return key
...
func _bm_apply_play_game_button_style(btn: Button, min_size: Vector2) -> void:
	if btn == null:
		return
	btn.custom_minimum_size = min_size
	var normal := StyleBoxFlat.new()
	normal.bg_color = Color(0.03, 0.16, 0.38, 0.98)
	normal.corner_radius_top_left = 14
	normal.corner_radius_top_right = 14
	normal.corner_radius_bottom_left = 14
	normal.corner_radius_bottom_right = 14
	normal.border_width_bottom = 4
	normal.border_color = Color(1.0, 0.05, 0.06, 0.92)
	normal.shadow_color = Color(1.0, 0.05, 0.06, 0.34)
	...
	hover.bg_color = Color(0.05, 0.24, 0.55, 0.98)
	hover.border_width_bottom = 6
	hover.border_color = Color(1.0, 0.12, 0.12, 1.0)
	hover.shadow_color = Color(1.0, 0.12, 0.12, 0.34)
	hover.shadow_size = 14
	var pressed := normal.duplicate() as StyleBoxFlat
	pressed.bg_color = Color(0.02, 0.11, 0.28, 1.0)
	pressed.border_width_bottom = 5
	pressed.border_color = Color(0.85, 0.02, 0.03, 1.0)
	pressed.shadow_color = Color(0.85, 0.02, 0.03, 0.34)
...
func _bm_place_management_top_left_buttons_stack() -> void:
	if BtnTabWinrates == null:
		return
	var base_pos := BtnTabWinrates.global_position
	if base_pos.x <= 1.0:
		base_pos.x = 24.0
	base_pos.y = 84.0
	var stack_index := -1
...
func _ensure_language_button() -> void:
	...
	BtnLanguage = Button.new()
	BtnLanguage.name = "BtnLanguage"
	BtnLanguage.text = tr("menu.languages")
	BtnLanguage.focus_mode = Control.FOCUS_NONE
	BtnLanguage.mouse_filter = Control.MOUSE_FILTER_STOP
	_bm_apply_play_game_button_style(BtnLanguage, Vector2(180, 48))

	var parent_ctrl := BtnTabWinrates.get_parent()
	if parent_ctrl == null:
		return
	parent_ctrl.add_child(BtnLanguage)
	BtnLanguage.set_as_top_level(true)
	_bm_place_management_top_left_buttons_stack()
...
func _on_btn_language_pressed() -> void:
	_show_management_language_popup()


func _show_management_language_popup() -> void:
	var old := get_node_or_null("ManagementLanguagePopup")
	if old != null:
		old.queue_free()
	...
	sb.border_color = Color(1.0, 0.62, 0.20, 0.90)
	...
	for item in [
		["fr", "res://assets/images/flags/fr.png"],
		["en", "res://assets/images/flags/us.png"],
		["es", "res://assets/images/flags/es.png"],
		["it", "res://assets/images/flags/it.png"],
		["pt", "res://assets/images/flags/pt.png"],
	]:
		var code := str(item[0])
		...
		var tex := load(str(item[1])) as Texture2D
		if tex != null:
...
func _bm_wr_ensure_visual_shell() -> void:
	if PanelWinrates == null:
		return

	if _bm_wr_dim == null or not is_instance_valid(_bm_wr_dim):
		_bm_wr_dim = ColorRect.new()
		_bm_wr_dim.name = "WinratesDim"
		_bm_wr_dim.set_anchors_preset(Control.PRESET_FULL_RECT)
		_bm_wr_dim.color = Color(0, 0, 0, 0.52)
		_bm_wr_dim.mouse_filter = Control.MOUSE_FILTER_STOP
		PanelWinrates.add_child(_bm_wr_dim)

	if _bm_wr_card == null or not is_instance_valid(_bm_wr_card):
		_bm_wr_card = PanelContainer.new()
		_bm_wr_card.name = "WinratesCard"
		_bm_wr_card.anchor_left = 0.12
		_bm_wr_card.anchor_top = 0.11
		_bm_wr_card.anchor_right = 0.88
		_bm_wr_card.anchor_bottom = 0.86
		_bm_wr_card.offset_left = 0
		_bm_wr_card.offset_top = 0
		_bm_wr_card.offset_right = 0
		_bm_wr_card.offset_bottom = 0
		_bm_wr_card.mouse_filter = Control.MOUSE_FILTER_STOP

		var sb := StyleBoxFlat.new()
		sb.bg_color = Color(0.03, 0.16, 0.38, 0.96)
		sb.border_width_left = 2
		sb.border_width_top = 2
		sb.border_width_right = 2
		sb.border_width_bottom = 2
		...
		_bm_wr_card.add_theme_stylebox_override("panel", sb)

		PanelWinrates.add_child(_bm_wr_card)

	PanelWinrates.move_child(_bm_wr_dim, 0)
	PanelWinrates.move_child(_bm_wr_card, min(1, PanelWinrates.get_child_count() - 1))
...
func _bm_wr_ensure_back_button() -> void:
	...
	if _bm_wr_back_btn == null or not is_instance_valid(_bm_wr_back_btn):
		...
		sbn.border_color = Color(1, 1, 1, 0.12)
		sbn.shadow_color = Color(0, 0, 0, 0.55)
		sbn.shadow_size = 14
		sbn.shadow_offset = Vector2(0, 8)

		var sbh := sbn.duplicate() as StyleBoxFlat
		sbh.bg_color = Color(0.89, 0.29, 0.337, 1.0)
...
# BM_SAVE_CHOICE_POPUP_V1
func _on_btn_save_pressed() -> void:
	_show_save_choice_popup()
...
func _show_save_choice_popup() -> void:
	if get_node_or_null("SaveChoicePopup") != null:
		return

	var popup := Control.new()
	popup.name = "SaveChoicePopup"
	popup.set_anchors_preset(Control.PRESET_FULL_RECT)
	popup.mouse_filter = Control.MOUSE_FILTER_STOP
	popup.z_index = 30000
	add_child(popup)
	...
	var title := Label.new()
	title.text = _tr_safe("menu.save_choice.title")
	...
	save_tip.name = "LblSaveChoiceTooltip"
	save_tip.text = ""
...
func _bm_is_mercato_open_now() -> bool:
	...
	print("[MERCATO][MENU] season_number=", season_number, " season_round=", season_round)

	if season_number < 2:
		return false

	if season_round == 0 or season_round == 1:
		return true
...
func _bm_mobile_start_by_building_text_plus2() -> void:
	...
	var btn := get_node_or_null("Overlays/PopupBienvenueClub/BtnCloseBienvenueClub") as Button
	if btn == null:
		btn = find_child("BtnCloseBienvenueClub", true, false) as Button
	if btn == null:
		return
...
# BM_GOAL_CLIMB_STANDINGS_ONCE_V1
func _bm_goal_is_mobile_layout() -> bool:
	var vp: Vector2 = get_viewport_rect().size
	var win: Vector2i = DisplayServer.window_get_size()
	return OS.has_feature("android") or OS.has_feature("ios") or minf(vp.x, vp.y) < 720.0 or mini(win.x, win.y) < 720
...
func _bm_goal_current_rank(save: Dictionary) -> int:
	...
	if ss != null and ss.has_method("get_current_club_rank"):
		return int(ss.call("get_current_club_rank", team_name))
...
func _bm_maybe_show_climb_standings_goal_once() -> void:
	...
	var round_played: int = int(save.get("season_round", 0))
	if save.has("progress") and typeof(save["progress"]) == TYPE_DICTIONARY:
		round_played = maxi(round_played, int((save["progress"] as Dictionary).get("journee", 1)) - 1)
	var goal_seen_key := "goal_climb_standings_seen"
	var goal_text_key := "goal.climb_standings_once"
	if bool(save.get(goal_seen_key, false)):
		return
	if round_played < 1:
		return

	save[goal_seen_key] = true
	PL.write_savegame(save)

	var vp: Vector2 = get_viewport_rect().size
	var mobile := _bm_goal_is_mobile_layout()
...
func _open_club_identity_popup() -> void:
	var existing := get_node_or_null("ClubIdentityPopup")
	if existing != null:
		existing.queue_free()
	...
	var popup := Panel.new()
	popup.name = "ClubIdentityPopup"
	popup.size = Vector2(520, 320)
	popup.position = (get_viewport_rect().size - popup.size) * 0.5
	popup.z_index = 999
	popup.mouse_filter = Control.MOUSE_FILTER_STOP
	popup.self_modulate = Color(1, 1, 1, 0.98)
	var popup_sb := StyleBoxFlat.new()
	popup_sb.bg_color = Color(0.015, 0.018, 0.030, 0.985)
	popup_sb.border_color = Color(0.16, 0.28, 0.48, 0.65)
	popup_sb.set_border_width_all(2)
	popup_sb.set_corner_radius_all(16)
	popup.add_theme_stylebox_override("panel", popup_sb)
	add_child(popup)

	var title_bg := Control.new()
	title_bg.position = Vector2(110, -26)
	title_bg.size = Vector2(300, 52)

	var title_shape_border := Polygon2D.new()
	title_shape_border.polygon = PackedVector2Array([
		Vector2(18, 0),
		Vector2(282, 0),
		Vector2(300, 16),
		Vector2(286, 52),
	...
	title_shape_border.color = Color(0.95, 0.08, 0.12, 1.0)
	title_bg.add_child(title_shape_border)

	var title_shape_inner := Polygon2D.new()
	title_shape_inner.position = Vector2(4, 4)
	title_shape_inner.polygon = PackedVector2Array([
		Vector2(16, 0),
		Vector2(268, 0),
		Vector2(284, 12),
		Vector2(272, 44),
	...
	title_shape_inner.color = Color(0.035, 0.16, 0.42, 1.0)
	title_bg.add_child(title_shape_inner)
	...
	info.add_theme_color_override("font_color", Color(0.88, 0.95, 1.0, 1.0))
	info.add_theme_color_override("font_outline_color", Color(0.02, 0.08, 0.16, 1.0))
	info.add_theme_constant_override("outline_size", 3)
	popup.add_child(info)

	var cost_capsule := Panel.new()
	cost_capsule.position = Vector2(360, 58)
	cost_capsule.size = Vector2(82, 38)
	var cost_sb := StyleBoxFlat.new()
	cost_sb.bg_color = Color(1.0, 0.56, 0.08, 1.0)
	cost_sb.border_color = Color(1.0, 0.86, 0.30, 1.0)
	cost_sb.set_border_width_all(2)
	cost_sb.set_corner_radius_all(18)
	cost_capsule.add_theme_stylebox_override("panel", cost_sb)
	popup.add_child(cost_capsule)

	var cost_row := HBoxContainer.new()
	cost_row.size = cost_capsule.size
	cost_row.alignment = BoxContainer.ALIGNMENT_CENTER
	cost_row.add_theme_constant_override("separation", 5)
	cost_capsule.add_child(cost_row)

	var cost_lbl := Label.new()
	cost_lbl.text = str(upgrade_cost)
	cost_lbl.vertical_alignment = VERTICAL_ALIGNMENT_CENTER
	cost_lbl.add_theme_font_size_override("font_size", 20)
	cost_lbl.add_theme_color_override("font_color", Color(1, 1, 1, 1))
	cost_row.add_child(cost_lbl)

	var cost_icon := TextureRect.new()
	cost_icon.texture = load("res://assets/images/token.png") as Texture2D
	cost_icon.custom_minimum_size = Vector2(24, 24)
	cost_icon.size = Vector2(24, 24)
	...
	if cid == "" and save.has("roster") and typeof(save["roster"]) == TYPE_DICTIONARY:
		cid = str((save["roster"] as Dictionary).get("selected_crest_id", "")).strip_edges()
	if cid != "":
		crest_idx = maxi(1, int(cid.replace("starter_crest_", "")))

	var lv2_tex := load("res://assets/images/blasons/blason_%d_lv2.png" % crest_idx) as Texture2D
	if lv2_tex != null:
		var lv2_preview := Sprite2D.new()
		lv2_preview.texture = lv2_tex
		var preview_size: float = 92.5
		var tex_size: Vector2 = lv2_tex.get_size()
		var max_side: float = maxf(tex_size.x, tex_size.y)
		if max_side > 0.0:
			var preview_scale: float = (preview_size * 1.6729625) / max_side
			lv2_preview.scale = Vector2(preview_scale, preview_scale)
		lv2_preview.position = Vector2(popup.size.x * 0.5, 182.0)
		popup.add_child(lv2_preview)

		var glow_tw := create_tween()
		glow_tw.set_loops()
		glow_tw.tween_property(lv2_preview, "modulate", Color(1.45, 1.35, 1.05, 1.0), 0.45)
	...
	cancel_sb.bg_color = Color(0.74, 0.10, 0.16, 1.0)
	cancel_sb.set_corner_radius_all(10)
	btn_cancel.add_theme_stylebox_override("normal", cancel_sb)
	btn_cancel.add_theme_stylebox_override("hover", cancel_sb)
	btn_cancel.add_theme_stylebox_override("pressed", cancel_sb)
	btn_cancel.pressed.connect(func():
		popup.queue_free()
	)
	...
	upgrade_sb.bg_color = Color(0.12, 0.62, 0.26, 1.0)
	upgrade_sb.set_corner_radius_all(10)
	btn_upgrade.add_theme_stylebox_override("normal", upgrade_sb)
	btn_upgrade.add_theme_stylebox_override("hover", upgrade_sb)
	btn_upgrade.add_theme_stylebox_override("pressed", upgrade_sb)
	var upgrade_msg := Label.new()
	upgrade_msg.text = ""
	upgrade_msg.position = Vector2(80, 36)
	upgrade_msg.size = Vector2(360, 26)
	upgrade_msg.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	upgrade_msg.add_theme_font_size_override("font_size", 18)
	upgrade_msg.add_theme_color_override("font_color", Color(1.0, 0.42, 0.30, 1.0))
	upgrade_msg.z_index = 10
	popup.add_child(upgrade_msg)

	btn_upgrade.pressed.connect(func():
		var before_tokens: int = PL.get_tokens(PL.load_savegame())
		_upgrade_club_crest_lv2()
		var after_save: Dictionary = PL.load_savegame()
		var after_tokens: int = PL.get_tokens(after_save)
		if after_tokens < before_tokens:
			_update_club_name_label_from_save()
			popup.queue_free()
		else:
			upgrade_msg.text = tr("club_identity.not_enough_tokens")
			var msg_tw := create_tween()
			msg_tw.tween_interval(4.0)
			msg_tw.tween_callback(func():
				if is_instance_valid(upgrade_msg):
					upgrade_msg.text = ""
			)
	)
	popup.add_child(btn_upgrade)


func _upgrade_club_crest_lv2() -> void:
	var save: Dictionary = PL.load_savegame()
	if typeof(save) != TYPE_DICTIONARY:
		return
