...
func _ensure_upgrade_dialogs() -> void:
	...
	if _upgrade_info_dialog == null:
		...
		ad.get_label().add_theme_font_size_override("font_size", 24)
		ad.add_theme_font_size_override("font_size", 24)
...
func _show_upgrade_info(message: String) -> void:
	...
	if _upgrade_info_dialog != null:
		...
		if _ok_btn != null:
			_ok_btn.add_theme_font_size_override("font_size", 24)
...
func _bm_refresh_price_adjust_mission_counter() -> void:
	...
	# Billetterie = 1 si les 3 prix sont renseignés
	var has_ticketing := false
	if PriceA != null and PriceB != null and PriceC != null:
		has_ticketing = _safe_int(PriceA.text) > 0 and _safe_int(PriceB.text) > 0 and _safe_int(PriceC.text) > 0
		...
		if typeof(items_any) == TYPE_DICTIONARY:
			...
			for pid2 in items.keys():
				...
				if typeof(row_any) == TYPE_DICTIONARY and int((row_any as Dictionary).get("price", 0)) > 0:
					...
					break
	if not has_shop:
		for pid3 in SHOP_DEFAULT_PRICES.keys():
			if int(SHOP_DEFAULT_PRICES.get(pid3, 0)) > 0:
				has_shop = true
				break
	if has_shop:
		count += 1
...
func _refresh_upgrade_works_ui(ng_cur: int, ns_cur: int, rem: int, total: int) -> void:
	...
	var btn_confirm := panel.get_node_or_null("VBoxUpgrade/BtnConfirmUpgradePanel") as Button
	...
	if frame != null:
		frame.custom_minimum_size = Vector2(780, 170)
	...
	if info != null:
		...
		info.text = "[center]" \
			...
			+ "\n" + _stadium_tr("stadium.duration") + " : " + str(total) + " match(s)" \
		...
		if LblStadiumLevel != null:
			...
			if info != null:
				...
				info.text = "[center]" \
					...
					+ "\n" + _stadium_fmt("stadium.upgrade.acceleration_available", {"tokens": accel_tokens}) \
	...
	if not bool(result.get("ok", false)):
		...
		if reason == "insufficient_funds":
			...
			if typeof(save_cur) == TYPE_DICTIONARY:
				...
				if dcur.has("wallet") and typeof(dcur["wallet"]) == TYPE_DICTIONARY:
					wallet_disponible = int((dcur["wallet"] as Dictionary).get("euros", 0))
...
func _shop_price_cap(pid: String) -> int:
	...
	var coef: float = 1.0
	if StadiumDataRef != null and StadiumDataRef.has_method("get_shop_price_cap_mult"):
