...
func _show_tournament_reward_popup(tournament_key: String, outcome: String) -> void:
	...
	var reward_key := _get_current_tournament_key()
	var reward: Dictionary = _grant_tournament_reward_once(reward_key, outcome)
	var euros_gain: int = _to_int_safe(reward.get("euros", 0))
	var tokens_gain: int = _to_int_safe(reward.get("tokens", 0))
	print("[INTERMEDIAIRE][REWARD] incoming_key=", tournament_key, " | forced_key=", reward_key, " | outcome=", outcome, " | euros=", euros_gain, " | tokens=", tokens_gain)
...
func _bm_has_tournament_result_any_season() -> bool:
	...
	for season_key in all_results.keys():
		...
		if not (season_results is Dictionary):
			continue
		var v := str(season_results.get("intermediaire", "")).strip_edges()
		if v != "":
			return true
...
func _bm_store_tournament_result(result_key: String) -> void:
	...
	season_results["intermediaire"] = result_key
...
func _maybe_show_tournament_victory_popup() -> void:
	...
	var subtitle := Label.new()
	subtitle.text = _tr_or("tournois.victory_subtitle_intermediate", "Victoire dans le tournoi Intermédiaire")
	subtitle.position = Vector2(30, 88)
	subtitle.size = Vector2(500, 34)
	subtitle.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	subtitle.add_theme_font_size_override("font_size", 24)
...
func _refresh_ui() -> void:
	if lbl_title != null:
		lbl_title.text = tr("tournois.intermediate")
