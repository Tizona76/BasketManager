...
func _bm_popup_intro_text() -> String:
	return _bm_tr_or_fallback("popup_match_title") + "\n\n" \
		+ _bm_tr_or_fallback("popup_match_play_title") + "\n" \
		+ _bm_tr_or_fallback("popup_match_play_body") + "\n\n" \
		+ _bm_tr_or_fallback("popup_match_season_title") + "\n" \
		+ _bm_tr_or_fallback("popup_match_season_body") + "\n\n" \
		+ _bm_tr_or_fallback("popup_match_extra_title") + "\n" \
		+ _bm_tr_or_fallback("popup_match_extra_body")
...
func _season_reward_menu_animate_tokens(lbl: Label, tokens_gain: int) -> void:
	...
	lbl.text = "+0"
	var displayed: int = 0
	while displayed < tokens_gain and is_instance_valid(lbl) and is_inside_tree():
		displayed += 1
...
func _ready() -> void:
	...
	# Mercato visible seulement à partir du niveau 3
	btn_mercato.visible = (SeasonState.club_level >= 1)

	if btn_close_bienvenue != null:
		btn_close_bienvenue.text = _bm_tr_or_fallback("popup_intro_close", "OK")
	...
	if lbl_bienvenue != null:
		...
		if _lines.size() > 0:
			...
			_lines[0] = "[center][font_size=22][b]" + _title + "[/b][/font_size][/center]"
		_txt = "\n".join(_lines)
		...
		var _body_fs = int(lbl_bienvenue.get_theme_font_size("normal_font_size"))
		lbl_bienvenue.add_theme_font_size_override("normal_font_size", _body_fs + 2)
	...
	popup_bienvenue.visible = !SeasonState.popup_bienvenue_saison_deja_vu
	...
	btn_calendrier.pressed.connect(func(): _select_zone("calendrier"))
	...
	btn_match.pressed.connect(_on_match_pressed)
...
func _get_end_season_summary() -> Dictionary:
	...
	var season_start_tokens := int(save.get("season_start_tokens", PL.get_tokens(save)))
	var xp_now := int(PL.get_club_xp(save))
	var tokens_now := int(PL.get_tokens(save))

	var xp_gain: int = max(0, xp_now - season_start_xp)
	var tokens_gain: int = max(0, tokens_now - season_start_tokens)
...
func _open_end_season_popup() -> void:
	...
	get_node("Overlays").add_child(popup_fin_saison)
...
func _missions_tr(key: String, fallback: String = "") -> String:
	...
	match key:
		...
		"missions.lv1.win_1":
			return "Gagner 1 match"
		"missions.lv1.pts_75_1":
			return "Marquer 75 points"
...
func _missions_get_level_data(level: int) -> Array:
	if TuningData.MISSIONS_BY_LEVEL.has(level):
		return TuningData.MISSIONS_BY_LEVEL[level]
	if TuningData.MISSIONS_BY_LEVEL.has(1):
		return TuningData.MISSIONS_BY_LEVEL[1]
	return []
...
func _missions_make_station(parent: Control, pos: Vector2, mission: Dictionary, state: String, idx: int) -> void:
	...
	if tex != null:
		...
		ball.position = pos - Vector2(24.84, 24.84)
		ball.size = Vector2(49.68, 49.68)
...
func _refresh_missions_panel() -> void:
	...
	if current_idx >= level_missions.size():
		...
		if btn_claim_mission != null:
			...
			btn_claim_mission.text = _missions_tr("missions.ui.done", "Terminé")
		return
	...
	if btn_claim_mission != null:
		btn_claim_mission.disabled = not done
		btn_claim_mission.text = (_missions_tr("missions.ui.claim", "Réclamer") if done else _missions_tr("missions.ui.in_progress", "Mission en cours"))
...
func _on_claim_mission_pressed() -> void:
	...
	save = PL.add_tokens(save, int(cur.get("reward_tokens", 0)), "mission_" + mid)
	PL.write_savegame(save)
	_refresh_missions_panel()
...
func _bm_is_mercato_open_now() -> bool:
	...
	print("[MERCATO][SAISON] season_number=", season_number)
