...
func _apply_i18n() -> void:
	...
	if hdr_stars != null:
		hdr_stars.text = tr("mercato.col.rank").to_upper()
	if hdr_poste != null:
		hdr_poste.text = tr("mercato.col.position_short").to_upper()
	if hdr_age != null:
		hdr_age.text = tr("mercato.col.age").to_upper()
	if hdr_fitness != null:
		hdr_fitness.text = tr("player.card.speed").to_upper()
	if hdr_tir != null:
		hdr_tir.text = tr("player.attr.tir")
	if hdr_perf != null:
		hdr_perf.text = tr("player.card.defense").to_upper()
	if hdr_accuracy != null:
		hdr_accuracy.text = tr("player.card.accuracy").to_upper()
	if hdr_motivation != null:
		hdr_motivation.text = _tr_any(["selection.header.motivation", "mercato.col.motivation"], "Motivation")
	if hdr_salaire != null:
		hdr_salaire.text = tr("mercato.col.salary").to_upper()
	if btn_retour != null:
