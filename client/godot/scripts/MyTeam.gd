...
func _bm_show_player_card_popup(data: Dictionary) -> void:
	...
	labels["age"] = _bm_player_card_add_label(card, _bm_myteam_tr_or_fallback("player.card.age", "Age") + " : " + str(int(float(data.get("age", 0)))), Vector2(254, 130), Vector2(210, 28), 20, Color(0.92, 0.95, 1.0, 1.0))
	labels["salary"] = _bm_player_card_add_label(card, _bm_myteam_tr_or_fallback("player.card.salary", "Salary") + " : " + _fmt_salary(_myteam_display_salary_annual_from_player(data)), Vector2(254, 160), Vector2(300, 28), 20, Color(0.92, 0.95, 1.0, 1.0))
	if data.has("pondération") or data.has("ponderation") or data.has("stars"):
		...
		labels["rating"] = _bm_player_card_add_label(card, _bm_myteam_tr_or_fallback("player.card.rating", "Rating") + " : " + rating, Vector2(254, 100), Vector2(260, 30), 22, Color(1.0, 0.78, 0.22, 1.0))
	...
	if bool(data.get("blessure", false)):
		labels["injured"] = _bm_player_card_add_label(card, _bm_myteam_tr_or_fallback("player.card.injured", "Injured"), Vector2(34, 292), Vector2(190, 30), 20, Color(1.0, 0.35, 0.35, 1.0), true)
