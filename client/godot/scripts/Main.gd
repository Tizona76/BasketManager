...
var _team_name_funnel_retry_scheduled: bool = false
var _team_name_funnel_in_flight: bool = false
...
func _show_accueil() -> void:
	...
	if _bm_is_mobile_layout():
		var vp := get_viewport_rect().size
		var mobile_landscape := vp.x > vp.y

		# BM_IOS_CLICK_START_LANDSCAPE_MINUS10_V2
		# Le VBoxContainer conserve la boîte de layout.
		# Réduction visuelle exacte de 10 % uniquement en paysage.
		btn.custom_minimum_size = Vector2(300, 72)
		btn.add_theme_font_size_override("font_size", 28)
		btn.pivot_offset = Vector2(150, 36)
		btn.scale = Vector2(0.90, 0.90) if mobile_landscape else Vector2.ONE
	else:
		btn.scale *= 1.2
	btn.disabled = false
	...
	if $UI.get_parent() == self:
		...
		print("[MAIN][SAFE] skip move_child UI parent=", $UI.get_parent())
...
func _on_click() -> void:
	...
	var _bm_mobile_web_intro := OS.has_feature("web") and _bm_is_mobile_layout()
	if _bm_mobile_web_intro:
		_audio_unlocked = true
		print("[AUDIO][MOBILE WEB] skip intro audio")
	else:
	...
	if _accueil_inst != null and _accueil_inst.has_method("start_intro"):
		if _accueil_inst.has_signal("intro_zoom_finished"):
			...
			if not _accueil_inst.is_connected("intro_zoom_finished", zoom_done):
				_accueil_inst.connect("intro_zoom_finished", zoom_done)
...
func _on_submit_team_name(team_name: String) -> void:
	...
	_track_team_name_created(team_name)
...
func _on_teamname_career_selected(career_id: String) -> void:
	...
	if active_before != cid:
		...
		print("[MAIN][CAREER] selected career already active: ", cid)
...
func _show_saison() -> void:
	...
	if season_scene == null:
		...
		return
	var season := season_scene.instantiate() as Control
	season.visible = false
	season.connect("back_to_management_requested", Callable(self, "_show_menu"), CONNECT_DEFERRED)
	$UI.visible = false
	_set_bg_visible(false)
	_clear_screen()
...
func _try_guest_auth_silent() -> void:
	print("[TRACE_FLOW] B GUEST_START")
...
func _is_reliable_funnel_profile_uuid(value: String) -> bool:
	var candidate := value.strip_edges()
	return candidate != "" and candidate != "default" and candidate != "TEST_UUID_EMPTY_001"




func _resolve_team_name_funnel_profile_uuid() -> String:
	var session_uuid := str(Session.profile_uuid).strip_edges()
	if _is_reliable_funnel_profile_uuid(session_uuid):
		return session_uuid

	var active_id := str(ProfileManager.get_active_profile_id()).strip_edges()
	if _is_reliable_funnel_profile_uuid(active_id):
		return active_id

	var save := PlayerLife.load_savegame()
	if typeof(save) == TYPE_DICTIONARY:
		for key in ["profile_uuid", "profile_id"]:
			var fallback := str(save.get(key, "")).strip_edges()
			if _is_reliable_funnel_profile_uuid(fallback):
				return fallback

	return ""


func _schedule_team_name_created_retry() -> void:
	if _team_name_funnel_pending == "" or _team_name_funnel_retry_scheduled:
		return

	if _team_name_funnel_retry_count >= TEAM_NAME_FUNNEL_MAX_RETRIES:
		print("[TEAM CREATED] GIVE UP profile_uuid unavailable team=", _team_name_funnel_pending)
		return

	_team_name_funnel_retry_count += 1
	_team_name_funnel_retry_scheduled = true

	var retry_timer := get_tree().create_timer(TEAM_NAME_FUNNEL_RETRY_DELAY_SECONDS)
	retry_timer.timeout.connect(func() -> void:
		_team_name_funnel_retry_scheduled = false
		if _team_name_funnel_pending != "":
			_track_team_name_created(_team_name_funnel_pending)
	)
...
func _track_team_name_created(team_name: String) -> void:
	...
	_team_name_funnel_pending = clean_team_name

	if _team_name_funnel_in_flight:
		return

	var profile_uuid := _resolve_team_name_funnel_profile_uuid()
	print("[TEAM CREATED] profile_uuid=", profile_uuid, " team=", clean_team_name)

	if profile_uuid == "":
		print("[TEAM CREATED] RETRY EMPTY PROFILE_UUID attempt=", _team_name_funnel_retry_count + 1)
		_schedule_team_name_created_retry()
		return

	var http := HTTPRequest.new()
	add_child(http)
	_team_name_funnel_in_flight = true

	http.request_completed.connect(func(result, code, _headers, body) -> void:
		_team_name_funnel_in_flight = false
		var ok: bool = result == HTTPRequest.RESULT_SUCCESS and int(code) >= 200 and int(code) < 300
		print("[TEAM CREATED][HTTP] result=", result, " code=", code, " body=", body.get_string_from_utf8())
		http.queue_free()

		if ok:
			_team_name_funnel_pending = ""
			_team_name_funnel_retry_count = 0
		else:
			_schedule_team_name_created_retry()
	)
